...
export class RoundTripComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getCarOptionsByCities(
      this.bookingData?.from_city_id,
      this.bookingData?.to_city_id
    );
  }
   ...
   */
  getCarOptionsByCities(from: number, to: number): void {
    this.loading = true;
    this.noCabsMessage = '';
    
    console.log('🔍 Fetching car options with:', { from, to });

    this.carservice.getRoundtripCab(from, to).subscribe({
      next: (response: any) => {
  ...
  getTabContent(car: CarOption): { icon: string; text: string }[] {
    const content = {
      inclusions: [
        { icon: 'fas fa-rupee-sign text-primary', text: `Base Fare for ${car.distance_km}` },
  ...
  onBookingUpdated(formData: any) {
    console.log('🔄 RoundTrip updated:', formData);
  
    // 1️⃣ bookingData update
