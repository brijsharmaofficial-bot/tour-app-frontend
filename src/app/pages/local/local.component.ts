...
import { CarService } from '../../services/car.service'; // Import the service
...
@Component({
  selector: 'app-local',
  standalone: true,
  imports: [CommonModule, NavbarComponent, FooterComponent,BookingFormModalComponent],
...
export class LocalComponent implements OnInit {
  ...
  packageOptions = [
    { label: '4 hrs | 40 km', hours: 4, distance: 40 },
    { label: '8 hrs | 80 km', hours: 8, distance: 80 },
    { label: '12 hrs | 120 km', hours: 12, distance: 120 }
  ];

  selectedPackage = this.packageOptions[0];
...
placesImages = [
  ...
  ngOnInit(): void {
    ...
    // Fetch car options dynamically using the API
    this.getCarOptionsByCity(this.bookingData.city_id, this.selectedPackage.hours, this.selectedPackage.distance);
  }
  ...
  getCarOptionsByCity(city_id: number, hours: number, distance: number): void {
    this.loading = true;
    this.noCabsMessage = '';

    this.carService.getLocalCab(city_id, hours, distance).subscribe({
      next: (response: any) => {
  ...
  selectPackage(pkg: any): void {
    ...
    this.getCarOptionsByCity(this.bookingData.city_id, pkg.hours, pkg.distance); // Refetch car options based on selected package
  }
  ...
  getTabContent(car: CarOption): { icon: string; text: string }[] {
    const content = {
      inclusions: [
        { icon: 'fas fa-rupee-sign text-primary', text: `Base Fare for ${car.included_km} km` },
      ...
      exclusions: [
        {icon:'fas fa-rupee-sign text-primary',text:`Base Fare for ${car.included_km} km. ₹${car.extra_price_per_km}/km will be charged beyond that`},
  ...
  onBookingUpdated(formData: any) {
    console.log('🔄 Local booking updated:', formData);
    ...
    // 3️⃣ Reset package (optional but recommended)
    this.selectedPackage = this.packageOptions[0];
  
    // 4️⃣ Re-call API with NEW data ✅
    this.getCarOptionsByCity(
      formData.city_id,
      this.selectedPackage.hours,
      this.selectedPackage.distance
    );
  }
